...
class ControllerProductSaveAfter implements ObserverInterface
{
    ...
    public function execute(EventObserver $observer)
    {
        ...
        $this->updateDefaultConfigStockForNewLocation($postData);
    }
    ...
    /**
     * Update Default Config for new location assigned to product
     *
     * @param $postData
     */
    private function updateDefaultConfigStockForNewLocation($postData)
    {
        $updateAttributes = [
            'manage_stock',
            'use_config_manage_stock'
        ];
        if (isset($postData['warehouse_stock_edit']['warehouse_stock_row'])) {
            $newAssignWareHouses = $postData['warehouse_stock_edit']['warehouse_stock_row'];
            foreach ($newAssignWareHouses as $newAssignWareHouse) {
                if (isset($newAssignWareHouse['warehouse_select']) && isset($postData['current_product_id'])) {
                    $wareHouseId = $newAssignWareHouse['warehouse_select'];
                    $productId = $postData['current_product_id'];
                    $stockItem = $this->warehouseStockRegistry->getStock($wareHouseId, $productId);
                    if ($stockItem && $stockItem->getItemId()) {
                        foreach ($updateAttributes as $attribute) {
                            if (isset($postData['stock_data'][$attribute])) {
                                $stockItem->setData($attribute, $postData['stock_data'][$attribute]);
                            }
                        }
                        $stockItem->save();
                    }
