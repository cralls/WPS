...
class AfterPlaceOrder implements ObserverInterface
{
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
       ...
       if($order->getStatus() == 'pending' && strpos($_SERVER['SERVER_NAME'], 'dev') === false) return true;
       
       //error_log(date('Y-m-d H:i:s')." - AfterPlaceOrder.php 1\r\n", 3, '/home/'.get_current_user().'/public_html/error_log');
       ...
       if($order->getWeltpixelFraudScore() > 4.99) {
           ...
           if(!$result) {
               //error_log(date('Y-m-d H:i:s')." - AfterPlaceOrder.php 2\r\n", 3, '/home/'.get_current_user().'/public_html/error_log');
            $result = $this->resendWps($order);
           }
    ...
    public function addPo($order) {
        ...
        foreach($simpleItems as $key => $item) {
            ...
            error_log("[".date('Y-m-d H:i:s')."] Order Item: ".$item->getSku()." - Dropshipped: ".$item->getDropshipped()." - Manufacturer: ".$product->getManufacturer()." - Type: ".$item->getProductType()." - Material: ".$product->getAttributeText('material')."\r\n", 3, '/home/'.get_current_user().'/public_html/var/log/addPo.log');
            ...
            if($product->getManufacturer() == 51 && $product->getAttributeText('material') != 'Assembly') {
                ...
                $added = false;
                if(count($purchaseOrderItems) > 0 && $product->getManufacturer() == 51) {
    ...
    public function sendPuOrder($order, $thirdParty = false) {
        ...
        // Check if the shipping address is in Alaska or Hawaii
        if (in_array($shippingRegionId, [2, 21])) {
            return false; // We don't dropship to HI or AK
        }
    ...
    public function resendWps($order) {
        try {
            $shippingAddress = $order->getShippingAddress();
            ...
            $cartPayload = [
                ...
                'ship_address1' => $shippingStreet[0],
                'ship_city' => $shippingAddress->getCity(),
                'ship_state' => $region,
                'ship_zip' => $shippingAddress->getPostcode(),
            ...
            //error_log(date('Y-m-d H:i:s')." - AfterPlaceOrder.php ".print_r($cartPayload, 1)."\r\n", 3, '/home/'.get_current_user().'/public_html/error_log');
            
            if(isset($shippingStreet[1])) $cartPayload['ship_address2'] = $shippingStreet[1];
            
            // Call function to create a cart
            $cartResponse = $this->createWpsCart($cartPayload);
            $cartResponse = $cartResponse['data'];
            
            if (!$cartResponse) {
                throw new \Exception("Failed to create WPS cart.");
            }
            ...
            foreach($itemCollection as $item) {
                ...
                if($item->getQtyRefunded() > 0) continue;
                $suppliers = $this->supplierCollection->addFieldToFilter('product_id', array('eq'=>$item->getProductId()));
                $this->supplierCollection->load();
                $productSupplierSku = '';
                if(count($suppliers) > 0) {
                    foreach($suppliers as $supplier) {
                        if($supplier->getSupplierId() == '91') {
                            $productSupplierSku = $supplier->getProductSupplierSku();
                            $addItemPayload = [
                                'item_sku' => $productSupplierSku,
                                'quantity' => number_format($item->getQtyOrdered())
                            ];
                            
                            $addItemResponse = $this->addItemToWpsCart($addItemPayload, $cartResponse['po_number']);
                            if ($addItemResponse && isset($addItemResponse['success']) && $addItemResponse['success']) {
                                $item->setDropshipped(1);
                            }
            ...
            // Send the order
            $this->sendWpsOrder($cartResponse['po_number']);
            
            // No parts unlimited products so we won't send the order to them
            if($o == 0) return false;
            
            // Log result
            error_log(date('Y-m-d H:i:s')." - RESEND REQUEST ".print_r($cartPayload, 1)."\r\n", 3, '/home/'.get_current_user().'/public_html/var/log/wps-orders.log');
            
            // Log result
            error_log(date('Y-m-d H:i:s')." - RESEND RESPONSE ".print_r($cartResponse, 1)."\r\n", 3, '/home/'.get_current_user().'/public_html/var/log/wps-orders.log');
            
            
            // Set order status
            if($pu && $npu) {
                $orderStatus = 'processing_wps_';
            } elseif($pu) {
                $orderStatus = 'processing_wps';
            }
            $order->setStatus($orderStatus);
            ...
            $order->save();
            //$this->messageManager->addSuccess(__('Order sent to WPS successfully'));
    ...
    private function sendWpsOrder($poNumber) {
        $apiUrl = "https://api.wps-inc.com/orders";
        $curl = curl_init($apiUrl);
        
        $orderPayload = ['po_number' => $poNumber];
        
        curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($curl, CURLOPT_POST, true);
        curl_setopt($curl, CURLOPT_POSTFIELDS, json_encode($orderPayload));
        curl_setopt($curl, CURLOPT_HTTPHEADER, [
            'Content-Type: application/json',
            'Authorization: Bearer ' . $this->variable->loadByCode('wps_api_key')->getPlainValue()
        ]);
        ...
        if ($httpCode != 200) {
            ...
            throw new \Exception("WPS API request for sending order failed with response: " . $response);
        }
    ...
    private function addItemToWpsCart($addItemPayload, $cartNumber) {
        $apiUrl = "https://api.wps-inc.com/carts/{$cartNumber}/items";
        $curl = curl_init($apiUrl);
        
        curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($curl, CURLOPT_POST, true);
        curl_setopt($curl, CURLOPT_POSTFIELDS, json_encode($addItemPayload));
        curl_setopt($curl, CURLOPT_HTTPHEADER, [
            'Content-Type: application/json',
            'Authorization: Bearer ' . $this->variable->loadByCode('wps_api_key')->getPlainValue()
        ]);
        ...
        if ($httpCode != 200) {
            // Handle non-200 responses
            throw new \Exception("WPS API request for adding item failed with response: " . $response);
        }
    ...
    private function createWpsCart($cartPayload) {
        $apiUrl = "https://api.wps-inc.com/carts";
        $curl = curl_init($apiUrl);
        
        curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($curl, CURLOPT_POST, true);
        curl_setopt($curl, CURLOPT_POSTFIELDS, json_encode($cartPayload));
        curl_setopt($curl, CURLOPT_HTTPHEADER, [
            'Content-Type: application/json',
            'Authorization: Bearer ' . $this->variable->loadByCode('wps_api_key')->getPlainValue()
        ]);
        ...
        //error_log(date('Y-m-d H:i:s')." - AfterPlaceOrder.php ".print_r($response, 1)."\r\n", 3, '/home/'.get_current_user().'/public_html/error_log');
        ...
        if ($httpCode != 200) {
            // Handle non-200 responses
            throw new \Exception("WPS API request failed with response: " . $response);
        }
