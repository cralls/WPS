...
sub vcl_recv {
    ...
    #    }
    #}
    # Updated condition to check for 'find' and 'sid' query parameters in the URL
    if (req.url ~ "\?(.*&)?find=[^&]+(&.*)?(&|$)") {
        return (pass);
    }
    ...
    # Bypass Parts Unlimited getPriceFile.php
    if (req.url ~ "/parts-unlimited/getPriceFile.php") {
        return (pass);
    }
    ...
    if (req.http.host == "dev.happy-trail.com") {
        return (pass);
    }
    if (req.http.host == "www.gpxplore.com") {
        return (pass);
    }
    if (req.http.host == "www.thrillparts.com") {
        return (pass);
    }
    if (req.http.host == "www.adventureparts.com") {
        return (pass);
    }
...
sub vcl_backend_response {
    ...
    # cache only successfully responses and 404s
    if (beresp.status == 200 || beresp.status == 404) {
        # Set a long TTL for these responses
        set beresp.ttl = 365d;  # Example: 1 year
    }
        # Set uncacheable for other status codes and private Cache-Control
    else {
        set beresp.uncacheable = true;
        return (deliver);
    }
